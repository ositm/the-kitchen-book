    ...
    <div className="flex flex-col gap-6 pb-24 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
            Real dishes shared by food lovers across Nigeria.
          </p>
        </div>

        <Link
          href="/post"
          className="bg-primary hover:bg-primary-dark text-white font-bold text-xs sm:text-sm px-4 py-2.5 rounded-2xl shadow-xs active:scale-95 transition-all flex items-center gap-1.5 flex-shrink-0"
        >
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="text-sm font-semibold text-foreground">Loading community dishes...</p>
        </div>
      ) : posts.length === 0 ? (
        <div className="bg-white p-10 rounded-3xl border border-[#EAE4D7] text-center text-muted-foreground space-y-3 shadow-xs">
          <span className="text-4xl block">🍳</span>
          <h3 className="text-base font-bold text-foreground">No dishes shared yet</h3>
          <p className="text-xs max-w-xs mx-auto">
            Be the first to share what you cooked with the community!
          </p>
          <Link
            href="/post"
        ...
        <div className="space-y-6">
          {posts.map((post) => {
            ...
            return (
              <div
                key={post.id}
                className="bg-white rounded-3xl overflow-hidden border border-[#EAE4D7] shadow-2xs food-card-hover transition-all"
              >
                {/* Post Header: Author & Location */}
                <div className="p-4 flex items-center justify-between border-b border-[#F0ECE3]">
      ...
      {activeCommentPostId && activePost && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-xs p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl border border-[#EAE4D7] flex flex-col max-h-[85vh] animate-in slide-in-from-bottom duration-200">
            {/* Drawer Header */}
              ...
              {(!activePost.comments || activePost.comments.length === 0) ? (
                ...
                activePost.comments.map((comment: any, i: number) => (
                  <div key={comment.id || i} className="flex items-start gap-3 text-xs sm:text-sm">
                    ...
                    <div className="bg-[#FAF7F2] p-3 rounded-2xl flex-1 border border-[#EAE4D7]">
            ...
            <div className="px-4 py-2 bg-[#FAF7F2] border-t border-[#F0ECE3] flex items-center justify-around text-lg">
              {["😍", "🔥", "😋", "👏", "❤️", "👍", "🤤"].map((emoji) => (
                <button
                  key={emoji}
                  type="button"
            ...
            {/* Comment Input Box */}
            <form onSubmit={handleAddComment} className="p-3 border-t border-[#EAE4D7] bg-white flex items-center gap-2">
              <input
                type="text"
                className="flex-1 py-2.5 px-4 bg-[#FAF7F2] border border-[#EAE4D7] rounded-xl text-foreground placeholder:text-muted-foreground text-xs sm:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/30"
